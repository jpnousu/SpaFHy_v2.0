...
from utils import peat_hydrol_properties, wrc

class Esom():

    """
    A class to simulate organic matter decomposition and nutrient dynamics
    ...
    sfc : numpy.ndarray
        An array representing the site fertility class, with integer values 
        between 1 and 6. The shape of this array should match the computational domain.
    days : int
        The length of the simulation in days.
    substance : str, optional
        Defines the process being simulated. Possible values:
        - 'Mass': Organic matter decomposition (default)
        - 'N': Nitrogen dynamics
    ...
    ----------
    substance : str
        The type of process being simulated, based on the substance parameter.
    
    Notes
    -----
    - The `Mass` instance handles the combined release of CO2 and DOC (Dissolved Organic Carbon)
      from organic matter, based on literature:
        - Laurén et al. 2012
        - Lappalainen et al. 2018
        - Laurén et al. 2019
    - The release rates for the 'Mass' instance:
        - L to out: 0.1 (DOC/CO2-C ratio)
        - F to out: 0.05
        - H to peat: 0.05
    - Further improvements are planned:
        - Consolidate `self.M` updates into a single method to allow external control.
        - Create an initialization loop to handle different layers with given
          parameters (e.g., water table depth, temperature, litterfall).
        - Modify initial values and layer boundaries to be shared between CO2 and DOC.
    """
    ...
    def __init__(self, spara, sfc, days, substance='Mass'):
        ...
        nutcpara = {'Mass': {'k1':1.05, 'k2': 1.05, 'k6':1.05},
        ...
        self.contpara = {'Mass': {2: {1:100., 2:100}, 3: {1:100., 2:100}, 4: {1:100., 2:100}, 5: {1:100., 2:100}},        
                    'N':{2: {1:1.9, 2:1.9}, 3: {1:1.6, 2:1.6}, 4: {1:1.4, 2:1.4}, 5: {1:1.2, 2:1.2}},        
        ...
        # Initial concentration of mass components in mor layer, gravimetric %
        self.contpara_mor = {'Mass': 100.0, 'N': 1.5, 'P': 0.05, 'K': 0.03}
    
        # pH according to site fertility class
        self.dph = {1:3.5, 2:3.4, 3:3.3, 4:3.2, 5:3.1, 6:3.0}
        ...
        # ---------this to array------------
        self.sfc_specification = 2 # MTkg1, MTkg2
        
        self.bound1 = 0.15 # boundary between top and middle layer, m
        self.bound2 = 0.4 # boundary between middle and bottom layers, m
        ...
        #temperature_functions (effect of temperature on the decomposition rate):
        self.t2 = interp1d([-40.,-5., -1, 25., 35., 60.],
                    [0.,   0.,  0.2, 1.53, 1.53, 0.])
        ...
        self.t6 = interp1d([-40., -5., 1., 27.5, 35., 60.],
                      [0., 0., 0.2, 1.95, 1.95, 0.])
        self.t7 = interp1d([-40., -30., -20. ,-10.,   0.,  10.,  20.,  30.,  40.,  50.],
                      [ 0.03125,0.0625, 0.125, 0.25, 0.5, 1., 2., 4., 8., 8.]) 
    
        # moisture_functions (description of Romul model Table 2)
    ...
    def reset_storages(self): 
        ...
        self.i = 0
        h_humus = 0.06      # Mor humus thickness in (m)
    ...
    def get_rates(self, tair, tp_top, tp_middle, tp_bottom, wn, peat_w1, peat_w2, peat_w3, H_w):
        ...
        tp_top : float
            Temperature at the top peat layer.
        tp_middle : float
            Temperature at the middle peat layer.
        tp_bottom : float
            Temperature at the bottom peat layer.
        wn : float
            Normalized water content (water content / field capacity).
        peat_w1 : float
            Water content in peat layer 1.
        peat_w2 : float
            Water content in peat layer 2.
        peat_w3 : float
            Water content in peat layer 3.
        H_w : float
            Water content in the humus layer.

        Returns
        -------
        tuple
            A tuple containing the decomposition rates for each layer (k1 to k9).
        """
    ...
    def decompose(self, k1, k2,  k3,  k4, k5, k6, k7, k8, k9, M):
        """
        Decompose carbon pools based on input rates and update the main storage matrix.

        Parameters
        ----------
        k1, k2, k3, k4, k5, k6, k7, k8, k9 : float
            Decomposition rates for different pools and peat layers.
        M : np.ndarray
            Main matrix containing 11 storages:
            0 - L0L: Leaf and fine root litter input.
            1 - L0W: Branch and coarse root litter input (woody).
            2 - LL: Leaf litter storage.
            3 - LW: Woody litter storage.
            4 - FL: Leaf F material storage.
            5 - FW: Woody F material storage.
            6 - H: Humus material storage.
            7 - P1: Peat storage (0-30 cm depth).
            8 - P2: Peat storage (30-60 cm depth).
            9 - P3: Peat storage (60 cm to bottom).
