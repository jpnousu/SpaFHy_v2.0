...
def parameters(folder=''):
    ...
    pgen = {'description': 'final_run',  # description written in result file
            ...
            'end_date': '2024-09-01', # 2017-12-31,
            'spinup_end': '2019-09-01',  # '2014-12-31', for tests: '2020-09-01' results after this are saved in result file
            'dt': 86400.0,
    ...
    pcpy = {
            ...
            'state': {  # spatial_cpy = False -> floats | spatial_cpy = True -> filenames in gispath
                       ...
                       'LAI_shrub': 0.1,
                       'LAI_grass': 0.2,
                       'canopy_height': 1.0, #'canopy_height.asc', # canopy height (m)
                       'canopy_fraction': 0.5, #'canopy_fraction.asc', # canopy closure fraction (-)
    ...
    pbu = {
            ...
            # rootzone layer
            'root_id': 'root_id.asc', # uniform (float) OR path to grid in gispath (str)     
    ...
    pspd = {
            ...
            'deep_id': 'deep_id.asc', # uniform (float) OR path to grid in gispath (str)
...
def ptopmodel():
    ...
    ptopmodel = {
            'dem': 'dem.asc',
            'flow_accumulation': 'flow_acc_dinf.asc',
            'slope': 'slope.asc',
            'twi': 'twi_dinf.asc',
